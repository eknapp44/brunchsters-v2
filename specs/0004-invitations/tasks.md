...
**Result after this pass:** 123 unit + 43 integration tests in `packages/core` (up from 118/42), 74 unit tests in `apps/web` (unchanged). `pnpm typecheck && pnpm lint && pnpm test` all green. Integration suite re-run against a real `postgres:17` Docker container (migrated + seeded fresh) since no local Supabase instance was running in this environment — all 43 pass, including the new revive-flow test confirming the fix against a real unique-constraint violation, not just mocks.

### CI: integration tests now run automatically (infra, not spec-scoped)

`docs/SETUP.md` had flagged integration-test CI wiring as "TBD when the first feature spec adds integration tests" — this spec is that first spec (7 `*.integration.test.ts` files across M1-M7). Added a `test-integration` job to `.github/workflows/ci.yml`: a `postgres:17` service container (version matches `supabase/config.toml`'s `[db] major_version`), migrated via `prisma migrate deploy` and seeded via `pnpm db:seed`, then `pnpm --filter @brunchsters/core test:integration`. Verified locally end-to-end (same commands, a throwaway Docker container) before trusting it in Actions — all 43 integration tests pass. `docs/SETUP.md` §8 updated to describe both the local mechanism (dev Supabase Postgres + `RUN_ID`-suffixed fixture data + `afterAll` cleanup, `packages/core/test/setup-db-env.ts` reads `DATABASE_URL` from `packages/database/.env` automatically) and the CI mechanism (fresh container per run, no cleanup needed), replacing the stale "dedicated `postgres_test` database" plan that was never actually built that way. This is infrastructure, not an invitations-feature change — called out here because it was motivated directly by this spec's review findings, but it isn't gated on this branch merging.

**Result:** test counts unchanged by this pass (still 123 unit + 43 integration in `packages/core`, 74 unit in `apps/web` — see the "Second review pass" result above); what changed is that the integration suite now runs automatically in CI instead of only on a developer's machine. `pnpm typecheck && pnpm lint && pnpm test` and `pnpm --filter @brunchsters/core test:integration` all green, both locally and (verified via a local throwaway Docker run of the same commands) in the new CI job.

### Third review pass (scoped to the two commits above, before merge)

A follow-up review scoped to just the "second review pass" fix and the CI-wiring commit (the original spec diff had already been fully reviewed) surfaced two real bugs and two doc inconsistencies:

- **`sendInvites`'s intra-batch de-duplication was still case-sensitive**, inconsistent with the case-insensitive host-filter added in the same commit right next to it. `Alice@Example.com` and `alice@example.com` in the same invite batch both survived the `Set`-based dedup (which compares raw strings) and would have produced two separate `BrunchInvite` rows for the same address, since `invitedEmail` has no case-insensitive DB constraint. Fixed by folding the dedup and host-filter into one case-insensitive loop that still preserves the first-seen casing for the stored value. New unit test: two same-address entries differing only in case collapse to one invite, using the first casing seen.
- **`InvitePanel`'s resend/revoke had no re-entrancy guard**, unlike `SuggestionsPanel.review()` in the same diff, which has `if (pendingSuggestionId !== undefined) return;`. Because `disabled` only takes effect after React re-renders, clicking resend on row A (disabling A) and then immediately clicking resend/revoke on row B before A's request resolved would reset the single shared `pendingInviteId` to B's id, re-enabling A's buttons mid-flight and allowing a second concurrent request against A. Fixed by adding the same guard to both `resend` and `revoke`.
- **This file's own "CI" section (the paragraph directly above) had a stale, copy-pasted `**Result after fixes:**` line** reporting 118/42 tests — a leftover copy from the earlier "PR review fixes" section two headings up, not updated for the actual (unchanged) 123/43 count. Fixed.
- **`docs/SETUP.md`'s "Test types" table still said "local Postgres (test DB)"** for Integration tests, contradicting the corrected "not a separate database" prose written two sections below it in the same commit. Fixed to just "local Postgres."

**Result:** 124 unit tests in `packages/core` (up from 123 — the one new dedup test), 43 integration tests (unchanged), 74 unit tests in `apps/web` (unchanged). Integration suite re-verified against a fresh `postgres:17` Docker container after the dedup fix — all 43 still pass. `pnpm typecheck && pnpm lint && pnpm test` all green.
